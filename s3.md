...
| `CopyObject`                 | PUT, bucket + key, `x-amz-copy-source`            | `s3:PutObject`, and `s3:GetObject` on the source |
...
| `UploadPartCopy`             | `UploadPart` with `x-amz-copy-source`             | `s3:PutObject`, and `s3:GetObject` on the source |
...
A copy operation reads a second bucket. A `PUT` of an object or a part whose `x-amz-copy-source` header parses is a copy; the header names the source as `bucket/key`, with an optional leading slash, URL-encoded, and an optional `?versionId=` suffix. A value with bad percent-encoding or no `/` separator is not a copy and the request classifies as the plain `PutObject` or `UploadPart` it otherwise is. The gateway validates the header's contents itself; the tenant service resolves and authorizes the bucket it names.
...
The gateway MAY authorize a request locally — without a round trip to the tenant service — when it holds a cached, unexpired derived signing key for the access key and, for every bucket the operation acts on (the addressed bucket and, for a copy, the source), the access key's S3 permissions as last returned by the tenant service and cached, unexpired delegation chains for every network command the required permission maps to. The permission check is required because several permissions map to the same commands (see [S3 permission mapping]): a chain alone cannot tell them apart. A gateway authorizing locally MUST apply the same operation classification and permission mapping as the tenant service, and MUST NOT trust an `x-amz-copy-source` header the signature does not cover.
...
7. For bucket-addressing operations: the bucket exists _(error name `UnknownBucket`)_, belongs to the tenant _(error name `ForeignBucket`)_, and the access key is scoped to it _(error name `BucketNotPermitted`)_. An access key with no bucket scope may use all of the tenant's buckets. Bucket names are global, so a bucket that exists under another tenant is reported as such rather than as unknown: the gateway answers as S3 does for another account's bucket.
8. For copy operations (`CopyObject`, `UploadPartCopy`): the `x-amz-copy-source` header is covered by the request signature _(error name `UnsignedCopySource`)_, the access key holds `s3:GetObject` _(error name `OperationNotPermitted`)_, and the source bucket resolves under the rules of step 7 _(error names `UnknownBucket`, `ForeignBucket`, `BucketNotPermitted`)_.

On success, the tenant service re-delegates the network commands the operation [maps to][S3 permission mapping] from the access key to the invocation issuer — one delegation per command, subject = the bucket DID, no policy. For a copy from another bucket it additionally re-delegates the commands `s3:GetObject` maps to, subject = the source bucket DID; a copy within one bucket is already covered by the destination's delegations.
...
type AuthorizeOK struct {
  ...
  sourceBucket optional String # source bucket DID (copies only)
  tenant       String          # tenant DID (the access key's tenant)
...
type AuthorizeOK struct {
	...
	SourceBucket *did.DID      `cborgen:"sourceBucket,omitempty"`
...
- `sourceBucket` is the DID of the bucket a copy operation reads from, as resolved and authorized in step 8. It MUST be present for `CopyObject` and `UploadPartCopy` (equal to `bucket` for a copy within one bucket) and MUST be absent otherwise. A gateway that caches per-bucket state from the response, such as the access key's permissions, keys it by the bucket DIDs the tenant service names.
- `tenant` is the DID of the tenant the access key belongs to (a `did:plc`, see [Tenant](#tenant)). It MUST be present. The gateway uses this DID to add the tenant as a recipient when encrypting stored objects.
  ...
  "args": {
    ...
    "out": {
      "ok": {
        ...
        "tenant": "did:plc:ewvi7nxzyoun6zhxrhs64oiz",
        "permissions": {
...
The result is the same structure as [Authorize Request] — the new bucket's DID, the tenant DID, the access key's permissions and derived signing key, and proof chains for any existing delegations that now grant access to the new bucket (e.g. powerline delegations). Delegation blocks travel in the response [container][UCAN container].

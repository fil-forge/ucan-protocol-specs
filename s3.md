...
- `tenant` is the DID of the tenant the access key belongs to (a `did:plc`, see [Tenant](#tenant)). It MUST be present. The gateway resolves the tenant's DID document and encrypts every stored object to the tenant wrap key (the verification method named `wrap`) as a recipient.
